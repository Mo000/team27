...
class objectDetection(object):
    ...
    def main(self):
        ...
        while not self.ctrl_c:
            ...
            while stage == 5:
                self.robot_controller.publish()
                if self.lidar['closest'] <= 0.33 and self.lidar['closest angle'] < 95:
                    self.robot_controller.set_move_cmd(linear = 0.0, angular = -0.5)

                if self.lidar['closest'] <= 0.33 and self.lidar['closest angle'] < 5:
                    self.robot_controller.set_move_cmd(linear = 0.0, angular = -0.5)
                ...
                if self.lidar['closest'] <= 0.33 and self.lidar['closest angle'] > 270:
                    self.robot_controller.set_move_cmd(linear = 0.0, angular = 0.5 )
