...
class SearchActionServer(object):
    ...
    def action_server_launcher(self):
        r = rospy.Rate(10)
        success = False

        #if rospy.Duration = 60:
            #success = True
        fwd_velocity = 0.15
        ...
        print("The robot is starting to explore...")
        # set the robot velocity:
        self.robot_controller.set_move_cmd(fwd_velocity, 0.0)
        self.robot_controller.publish()

        #TODO decide on robot movement rules
        while success == False:
            if self.min_distance <= approach_distance:
                #is object on the right? turn left a bit
                if self.object_angle >= 0:
                    while self.min_distance <= approach_distance:
                        self.robot_controller.set_move_cmd(fwd_velocity, 0.2)
                    self.robot_controller.set_move_cmd(fwd_velocity, 0.0)
                else: #otherwise turn right a bit
                    while self.min_distance <= approach_distance:
                        self.robot_controller.set_move_cmd(fwd_velocity, -0.2)
                    self.robot_controller.set_move_cmd(fwd_velocity, 0.0)
